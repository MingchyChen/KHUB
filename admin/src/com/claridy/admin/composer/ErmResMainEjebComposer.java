...
public class ErmResMainEjebComposer extends SelectorComposer<Component> {
	/**
	 * 
	 */
	private static final long serialVersionUID = 717212601507168916L;
	...
	@Wire
	private Window editErmResEjebWin;
	@Wire
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		try {
			...
			numPage = map.get("numPage");// 為0是清單列的編輯 1為附檔列編輯
			String resTitleId = map.get("resTitleId");
			if (resTitleId != null && !"".equals(resTitleId)) {
				...
				if (str.length == 1) {
					...
					dbId=resourcesMainfile.getDbId();
				} else {
					resourcesMainfile = ((ErmResMainEjebService) SpringUtil
							.getBean("ermResMainEjebService"))
							.getErmResMainfileVByResIdAndDbid(resourcesId, dbId);
				}
			}
			// resourcesMainfile = ((ErmResMainEjebService) SpringUtil
			// .getBean("ermResMainEjebService"))
			...
			if (resourcesMainfile != null) {
				...
				title.setValue(resourcesMainfile.getTitle());// 題名
				// isOpen.setSelectedIndex(resourcesMainfile.getCore());
				if ("Y".equals("resourcesMainfile.getCore()")) {
					isOpen.setSelectedIndex(0);// 核心
				} else {
					isOpen.setSelectedIndex(1);
				}
				reportLink.setValue(resourcesMainfile.getUrl1());// url
				remarks.setValue(resourcesMainfile.getUrl2());// 相關url

				// langTbx.setValue(resourcesMainfile.getLanguageId());// 語言
				...
				// pubTbx.setValue(resourcesMainfile.getPublisherId()); //出版商
				publisherSearch.setValue(resourcesMainfile.getPublisherId());
				ErmCodePublisher ermCodePublisher = ((CodePublisherService) SpringUtil
						.getBean("codePublisherService"))
						.findByPublisherID(resourcesMainfile.getPublisherId());
				if (ermCodePublisher != null) {
					pubTbx.setValue(ermCodePublisher.getName());
				}
				if (resourcesMainfile.getIssnprinted() != null) {
					issnPringtedTxt
							.setValue(resourcesMainfile.getIssnprinted());
				}
				if (resourcesMainfile.getIssnonline() != null) {
					issnOnlineTxt.setValue(resourcesMainfile.getIssnonline());
				}

				// agenTbx.setValue(resourcesMainfile.getDbId());//所屬資料庫
				// ErmCodeDbService
				agentedSearch.setValue(resourcesMainfile.getDbId());
				ErmCodeDb ermCodeDb = ((ErmCodeDbService) SpringUtil
						.getBean("ermCodeDbService")).findcodeDbByDbId(
						resourcesMainfile.getDbId(), webEmployee);
				if (ermCodeDb.getName() != null
						&& !"".equals(ermCodeDb.getName())) {
					agenTbx.setValue(ermCodeDb.getName());
				}

				startOrderDateDtx
						.setValue(resourcesMainfile.getStarOrderDate());// 起訂開始日期
				endOrderDateDtx.setValue(resourcesMainfile.getEndOrderDate());// 訖訂結束日期
				// procurementNoteCbx.setValue(resourcesMainfile.getRemarkId());//
				// 採購註記
				if (resourcesMainfile.getRemarkId() != null
						&& !"".equals(resourcesMainfile.getRemarkId())) {
					procurementNoteCbx.setSelectedIndex(Integer
							.parseInt(resourcesMainfile.getRemarkId()));
				}
				if (resourcesMainfile.getLibaryMoney() != null) {
					libaryMoneyRdo.setSelectedIndex(Integer
							.parseInt(resourcesMainfile.getLibaryMoney()));// 圖書館經費
				}
				publishedInTxt.setValue(resourcesMainfile.getPubPlace());// 出版地
				idPwdTxt.setValue(resourcesMainfile.getIdpwd());// 賬號與密碼
				// locationCbx.setValue(resourcesMainfile.getPlaceId());// 存放地點
				if (resourcesMainfile.getPlaceId() != null
						&& !"".equals(resourcesMainfile.getPlaceId())) {
					locationCbx.setSelectedIndex(Integer
							.parseInt(resourcesMainfile.getPlaceId()));
				}
				includedTxt.setValue(resourcesMainfile.getCoverage());// 收錄年代
				updateFrequency.setValue(resourcesMainfile.getFrenquency());// 更新頻率/刊期
				legitimateIPTxt.setValue(resourcesMainfile.getRegllyIp());// 合法IP
				embargoTxt.setValue(resourcesMainfile.getEmbargo());// embargo
				collectionInfoTxt.setValue(resourcesMainfile.getEholdings());// 館藏資訊
				authorTxt.setValue(resourcesMainfile.getAuthor());// 作者
				callnTxt.setValue(resourcesMainfile.getCalln());// 電子書索書號
				cnTxt.setValue(resourcesMainfile.getCn());// 電子書分類號
				versionTxt.setValue(resourcesMainfile.getVersion());// 版本
				publisherUrlTxt.setValue(resourcesMainfile.getPublisherUrl());// 出版商首頁
				othersTxt.setValue(resourcesMainfile.getOthers());// 其他資訊
				brief1CKtxt.setValue(resourcesMainfile.getBrief1());// 資源簡述摘要
				brief2CKtxt.setValue(resourcesMainfile.getBrief2());// 資源簡述摘要(英文)
				if (resourcesMainfile.getImgurl() != null
						&& !"".equals(resourcesMainfile.getImgurl())) {
	...
	public void addResourcesDbws() {
		try {
			...
			resourcesMainEjeb.setCreateDate(new Date());
			resourcesMainEjeb.setWebEmployee(webEmployee);
			resourcesEjebItem.setCreateDate(new Date());
			resourcesEjebItem.setWebEmployee(webEmployee);
	...
	public void editErmResEjebList() {
		...
		try {
			Listbox webEjebLix = (Listbox) editErmResEjebWin.getParent()
					.getFellowIfAny("resMainEjebLix");
			Label languageSearch = (Label) editErmResEjebWin.getParent()
					.getFellowIfAny("languageSearch");
			Textbox titleBox = (Textbox) editErmResEjebWin.getParent()
					.getFellow("titleBox");
			Label publisherSearch = (Label) editErmResEjebWin.getParent()
					.getFellowIfAny("publisherSearch");
			Label agentedSearch = (Label) editErmResEjebWin.getParent()
					.getFellowIfAny("agentedSearch");
			Datebox startDateDbx = (Datebox) editErmResEjebWin.getParent()
					.getFellowIfAny("startDateDbx");
			Datebox endDateDbx = (Datebox) editErmResEjebWin.getParent()
					.getFellowIfAny("endDateDbx");
			Combobox remarkCbx = (Combobox) editErmResEjebWin.getParent()
					.getFellowIfAny("remarkCbx");
			Combobox stateCbx = (Combobox) editErmResEjebWin.getParent()
					.getFellowIfAny("stateCbx");
			Radiogroup historyRgp = (Radiogroup) editErmResEjebWin.getParent()
					.getFellowIfAny("historyRgp");
			Textbox subjectIdBox = (Textbox) editErmResEjebWin.getParent()
					.getFellow("subjectIdBox");
			Combobox providerCbx = (Combobox) editErmResEjebWin.getParent()
			...
			search(languageSearch, titleBox, publisherSearch, agentedSearch,
					...
					webEjebLix, Integer.parseInt(currentPage), subjectIdBox,
					providerCbx);
		} catch (Exception e) {
			// TODO: handle exception
	...
	public void toResourcesPg() {
		...
		editErmResEjebWin.detach();
		editErmResEjebWin = (Window) ZkUtils.createComponents(
				"/WEB-INF/pages/system/ermResMainEjeb/ermResEjebMainten.zul",
				null, map);
		editErmResEjebWin.doModal();
	}
