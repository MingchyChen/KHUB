...
public class ErmResMainDbwsOpenComposer extends SelectorComposer<Component>{

	/**
	 * 
	 */
	private static final long serialVersionUID = 6153852920249025283L;
	...
	@Wire
	private Window ermResMainDbwsOpenWin;
	...
	public void selectBtn() {
		...
		if (size == 0) {
			...
			for (Listitem tmpEST : selectedModels) {
				...
				Textbox resourcesId=(Textbox) ermResMainDbwsOpenWin.getParent().getFellowIfAny("resourcesIdTxt");
				Textbox resourcesName=(Textbox) ermResMainDbwsOpenWin.getParent().getFellowIfAny("resourcesNameTxt");
				resourcesId.setValue(ermResourcesMainDbws.getResourcesId());
				resourcesName.setValue(ermResourcesMainDbws.getTitle());
				ermResMainDbwsOpenWin.detach();
			}
		}
	}
