...
@Service
public class CooperUnitsService {
	@Autowired
	...
	public List<ApplyCooperRes> findApplyCooperResList(Date startDate, Date endDate){
		...
		List<Object> webCoopertList=webCooperationDao.findCooperUnitsList(sdf.format(startDate), sdf.format(endDate));
		for (int i = 0; i < webCoopertList.size(); i++) {
			...
			for (WebOrg webOrg:parentOrgList) {
				if(webOrg.getOrgId().equals(parentOrgId)){
					...
					List<Object> findCooperNumberList=webCooperationDao.findCooperUnitsNumberList(sdf.format(startDate),  
					sdf.format(endDate),parentOrgId);
					...
					for (int j = 0; j < findCooperNumberList.size(); j++) {
						...
						String status=coopObj[1].toString();
						int number=Integer.parseInt(coopObj[2].toString());
						if(status.equals("0")){
	 ...
	 * @return
	 */
	public List<WebCooperation> findWebCooperationListByStatus(Date startDate, Date endDate,String parentOrgId,String status){
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		List<WebCooperation> webCoopertList=webCooperationDao.findCooperUnitsListByStatus(sdf.format(startDate), sdf.format(endDate),parentOrgId,status);
