...
@Repository
public class ErmResUnitUseDAO extends BaseDAO implements IErmResUnitUseDAO {
	@Autowired
	...
	@Autowired
	public ErmResUnitUseDAO(HibernateTemplate hibernateTemplate) {
		setHibernateTemplate(hibernateTemplate);
	}

	@SuppressWarnings("unchecked")
	public List<Object> findErmResourcesRsconAll(String tempstartDateDbx, String tempendDateDbx) {
	...
	@SuppressWarnings("unchecked")
	public List<Object> findErmResUnitList(String tempstartDateDbx, String tempendDateDbx, String resId) {
		Session session = this.getSession();
		String hql = "";
		Query query = null;
		hql = "select accountid,resources_id,CONVERT(VARCHAR(8),createdate,112) as datetime,db_id,parentorgid,orgid from erm_personalize_rescount where CONVERT(VARCHAR(8),createdate,112) between'"
	...
	@SuppressWarnings("unchecked")
	public List<Object> findErmResUnitListBydata(String tempstartDateDbx, String tempendDateDbx, String resId, String dbId) {
		Session session = this.getSession();
		String hql = "";
		Query query = null;
		hql = "select accountid,resources_id,CONVERT(VARCHAR(8),createdate,112) as datetime,db_id,parentorgid,orgid from erm_personalize_rescount where CONVERT(VARCHAR(8),createdate,112) between'"
	...
	@SuppressWarnings("unchecked")
	public List<Object> findResIdAll(String tempstartDateDbx, String tempendDateDbx) {
		Session session = this.getSession();
		String hql = "";
		Query query = null;
		hql = "select resources_id,COUNT(*) from erm_personalize_rescount where CONVERT(VARCHAR(8),createdate,112) between'"
				+ String.valueOf(tempstartDateDbx) + "' and '" + String.valueOf(tempendDateDbx) + "' group by resources_id";
		query = session.createSQLQuery(hql);
		List<Object> list = query.list();
		return list;
	}
	...
	public ErmCodeDb findDbId(String dbName) {
		...
		StringBuffer sbHql = new StringBuffer();
		sbHql.append(" from ErmCodeDb ");
		sbHql.append(" where name ='" + dbName + "'");
		Query query = this.getSession().createQuery(sbHql.toString());
		List<ErmCodeDb> ermCodeDbList = query.list();
		if (ermCodeDbList.size() > 0) {
			ermCodeDb = ermCodeDbList.get(0);
	...
	public List<ErmResourcesMainfileV> findResAll() {
		StringBuffer sbHql = new StringBuffer();
		sbHql.append("from ErmResourcesMainfileV ");
		sbHql.append(" where isDataEffid = 1");
		Query query = this.getSession().createQuery(sbHql.toString());
		return query.list();
	}
